...
void send(vector<vector<int>>* polies, MPI_Datatype MPI_Poly, int which_pr);

void fillPoly(vector<vector<int>>* polies, int how_many_needed)
{
	int poly_size = MAXSIZE;
	vector<int> res_poly(MAXSIZE, 0);
	for (int i = 0; i < how_many_needed; i++)
		polies[0].push_back(createPoly(rand() % 6 + 1));
	for (int i = 0; i < polies[0].size(); i++)
	{
		polies[0][i].resize(MAXSIZE);
	}
}
...
int main(int* argc, char* argv[])
{
	...
	MPI_Comm_rank(MPI_COMM_WORLD, &rank); // proper names needed 
	...
	if (rank == 0)
	{
		...
		fillPoly(&polies, full_size);
		for (int i = 0; i < full_size; i++)
			printPoly(polies[i]);
		for (int i = 1; i < size; i++)
		{
			send(&polies, MPI_Poly, i);
			Sleep(100);
		}
		if (polies.size() < 2)
		{
...
{
	cout << "first: ";
	printPoly(polies[0].back());

	MPI_Send(&polies[0].back().front(), 1, MPI_Poly, which_pr, 1, MPI_COMM_WORLD);
	polies[0].pop_back();
	cout << "second: ";
	printPoly(polies[0].back());
	MPI_Send(&polies[0].back().front(), 1, MPI_Poly, which_pr, 1, MPI_COMM_WORLD);
...
// print out full vectors manually, check truncation